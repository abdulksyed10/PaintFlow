    ...
    <Card className="rounded-2xl border-black/5 shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">Manage storefront and catalog categories locally.</p>
        </div>
        ...
        <table className="w-full min-w-190 text-left text-sm">
          ...
          <tbody>
            {categories.map((category) => (
              <tr key={category.id} className="border-b border-black/5 last:border-b-0">
                ...
                <td className="px-3 py-4 capitalize text-neutral-600">{category.categoryType}</td>
                <td className="px-3 py-4">
                  <Badge variant={category.storefrontVisible ? "default" : "secondary"}>
                    {category.storefrontVisible ? "Visible" : "Hidden"}
                  </Badge>
                </td>
                <td className="px-3 py-4 text-neutral-600">{category.displayOrder}</td>
                <td className="px-3 py-4">
